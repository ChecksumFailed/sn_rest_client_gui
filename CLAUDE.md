...
- `src/fluent/security/` — `roles.now.ts` (`Role` `x_1676196_rest_gui.user`, the `EXPLORER_ROLE`) and `properties.now.ts` (four `Property` records: `default_mid_server` the fallback MID server, `enable_direct_url` the Direct URL mode kill switch, `debug` gates request-body capture in the audit log, `sensitive_query_params` the redaction denylist — all admin-write except `default_mid_server`).
- `src/fluent/table/audit-log.now.ts` — `Table` `x_1676196_rest_gui_audit_log`, written by `RestExplorerEngine._logRequest` on every `execute()` call. Stores the full request URL with values of the query params named in the `sensitive_query_params` property redacted (`_redactSensitiveQueryParams`); never stores headers; stores the request body only when `debug` is on; `oauth_profile`/`basic_auth_profile` (references) record which stored credential profile a call used (never both, and neither for manual-entry Basic, API key, or no auth). Read is row-scoped via the `auditLogReadOwnOnly` `Acl` in the same file — `x_1676196_rest_gui.user` holders see only rows they created (`sys_created_by == gs.getUserName()`), admins see all rows (`adminOverrides`).
  - **Retention:** a Table Cleaner (`sys_auto_flush`, paired with a `sys_dm_policy` header) purges rows after 14 days on `sys_created_on`. Both were authored on the dev instance via System Definition > Table Cleanup / Data Management, then pulled into `src/fluent/generated/other/sys-auto-flush/` and `src/fluent/generated/other/sys-dm-policy/` by `now-sdk build`'s auto-sync (the same mechanism that produces `src/fluent/generated/other/sys-scope-privilege/`) — **generated, do not hand-author a duplicate Fluent `Record()` for these**; change the retention window on the instance and re-run `npm run build` to re-pull it.
  - **Default view form:** explicitly defined via the `auditLogForm` `Form()` in the same file (one "Details" section, sys_id `a9e2f40625874432b31f529ef448164f`, wired through `sys_ui_form`/`sys_ui_form_section`). This exists because `Table()` alone does not create that wiring.
  - **History note (resolved):** the table had a *pre-existing* `sys_ui_section` on the dev instance (sys_id `c208054093120b10e4bb7ee32bba100b`) that predated `auditLogForm` and rendered as an unlabeled duplicate stacked on top of the real form. It wasn't just live-instance drift — two files committed in `47903ed` (`src/fluent/generated/data/form-section/sys_ui_section_ce088dcc93dec710e4bb7ee32bba10c5.now.ts` and `src/fluent/generated/other/sys-ui-element/sys_ui_section_ce088dcc93dec710e4bb7ee32bba10c5.now.ts`) actively re-declared that section and its 20 elements on *every* build, so deleting it on the instance alone didn't stick — the next deploy just recreated it. Both files are now deleted from source (build confirms no trace of `c208...` in `dist/` anymore, and `now-sdk build`'s auto-sync did not resurrect them). **If a duplicate section is still visible on an instance**, that's leftover live data from before this fix — the update set no longer touches it, so it needs one manual deletion via System UI > Sections (`table=x_1676196_rest_gui_audit_log`, delete anything that isn't `a9e2f40625874432b31f529ef448164f`); it will not come back after that, since nothing in source recreates it anymore.
- `src/fluent/generated/keys.ts` — generated `Now.ID` key registry; do not hand-edit.
- `src/fluent/generated/other/**` — generated mirror of instance customizations not otherwise represented in hand-authored Fluent source (cross-scope privileges, the audit log's Table Cleaner/Data Management Policy, etc.), produced by `now-sdk build`'s auto-sync; do not hand-edit.
