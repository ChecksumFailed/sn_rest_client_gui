...
- `src/fluent/table/audit-log.now.ts` — `Table` `x_1676196_rest_gui_audit_log`, written by `RestExplorerEngine._logRequest` on every `execute()` call. Stores the full request URL with values of the query params named in the `sensitive_query_params` property redacted (`_redactSensitiveQueryParams`); never stores headers; stores the request body only when `debug` is on; `oauth_profile`/`basic_auth_profile` (references) record which stored credential profile a call used (never both, and neither for manual-entry Basic, API key, or no auth). Read is row-scoped via the `auditLogReadOwnOnly` `Acl` in the same file — `x_1676196_rest_gui.user` holders see only rows they created (`sys_created_by == gs.getUserName()`), admins see all rows (`adminOverrides`).
  ...
  - **Default view form:** explicitly defined via the `auditLogForm` `Form()` in the same file (one "Details" section, wired through `sys_ui_form`/`sys_ui_form_section`). This exists because `Table()` alone does not create that wiring, and the table already had a *pre-existing* `sys_ui_section` on the dev instance (sys_id `c208054093120b10e4bb7ee32bba100b`, mirrored into `src/fluent/generated/other/sys-ui-element/` by the same auto-sync mechanism as the Table Cleaner, frozen at whatever the schema looked like when first pulled) that rendered as an unlabeled duplicate stacked on top of the real form. **One-time manual cleanup required on any instance carrying that stray section:** System UI > Sections, filter `table=x_1676196_rest_gui_audit_log`, delete the section whose sys_id is *not* `a9e2f40625874432b31f529ef448164f` (the `auditLogForm`-managed one, captioned "Details") — deploying this update set alone won't remove it, since update sets never delete records they no longer reference.
